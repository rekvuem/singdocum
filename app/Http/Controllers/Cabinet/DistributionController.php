...
class DistributionController extends BaseController {
  ...
  public function index() {
    ...
    $listToken = DB::table('document_toview')->where('users_id', Auth::id())->first();
    ...
    return view('cabinet.distrib.list', compact(['listDocument', 'typeOfDocum']));
  }

  public function checking($token, $type) {
    $takeid = $token;
    $ToView = DB::table('document_toview')
        ->where('document_id', $token)
        ->where('users_id', Auth::id())
        ->first();
    IF ($ToView->view_status == 'no view')
    {
      DB::table('document_toview')
          ->where('users_id', Auth::id())
          ->update([
            'view_status' => 'view',
            'date_view'   => now()
